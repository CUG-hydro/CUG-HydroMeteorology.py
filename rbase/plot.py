...
def setMapAxis_China(ax,
                     lon=np.arange(70, 170, 10),
                     lat=np.arange(5, 65, 5)):
    ax.set_extent([70, 160, 5, 55])
    return setMapAxis(ax, lon, lat)


def get_cmap(cmap=cmaps.amwg256, ncol=12,
             under=np.array([1, 1, 1, 1]), extend="both"):
    """
    # References
...
def contourf_prcp(ax, prcp):
    bounds = [0.1, 0.2, 0.5, 1, 2, 5]
    ncol = (len(bounds) - 1) * 2
    # cmap = cmaps.amwg256
    cmap = get_cmap(ncol=ncol)
    ax.contourf(prcp.lon.values, prcp.lat.values, 
        prcp,
        levels=bounds,
        cmap=cmap,
...
def plot_maxmin_points(ax, xarr, nsize = 12, 
